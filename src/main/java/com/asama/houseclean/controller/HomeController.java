...
import com.asama.houseclean.bean.MailInfo;
...
public class HomeController {
    ...
    public String request(Model model, @PathVariable("id") Long id) {
        ...
        if (guestRequest.isPresent()) {
            ...
            model.addAttribute("message",
                    "Xác nhận thông tin đặt hẹn <b>thành công</b>, chúng tôi sẽ sớm liên lạc với bạn thông qua số điện thoại bạn đã cung cấp. Thanks !");
        } else {
            model.addAttribute("message",
                    "Xác nhận thông tin đặt hẹn <b><i>không thành công</i></b>. Lịch hẹn này không tồn tại hoặc đã được xử lý. Thanks !");
        }
    ...
    public String request(RedirectAttributes re, HttpServletRequest request, Model model,
            @Validated @ModelAttribute("req_book") GuestRequest guestRequest, BindingResult errors)
        ...
        if (errors.hasErrors()) {
            ...
            model.addAttribute("times", times);
            return "home/quotes";
        } else if (!reCaptchaService.validate(request.getParameter("reCaptchaResponse"))) {
        ...
        String body = "Bạn vừa yêu cầu đặt lịch hẹn Tư vấn và Khảo sát tại trang web " + "Vệ sinh nhà sạch Pleiku, "
                + "<br>Vui lòng click vào đây để <a href='" + url + "'>Xác nhận</a> <br>"
                + "Nếu không phải là bạn, xin hãy bỏ qua email này.<br>" + "<b>Nhà sạch Pleiku</b><br>"
                + "Chúc bạn một ngày tốt lành ! " + "Trân trọng.";
        MailInfo mailInfo = new MailInfo(from, to, subject, body);

        mailService.send(mailInfo);

        re.addAttribute("message", "Đã gửi yêu cầu, vui lòng vào email để xác nhận !");
        return "redirect:/home/request/book";
    }
